...
public class SanctionChangesRecord implements DTOObject<SanctionChangesEntity> {
    ...
    private SanctionChangesRecord mainSanction;
    ...
    public SanctionChangesRecord getMainSanction() {
        return mainSanction;
    }

    public void setMainSanction(SanctionChangesRecord mainSanction) {
    ...
    public SanctionChangesEntity toDB(boolean deepCopy) {
        ...
        if (this.getAlternateSanctions() != null && deepCopy) {
            record.setAlternateSactions(this.getAlternateSanctions().stream().map(DTOObject::toDB).collect(Collectors.toList()));
        }
        if (this.getMainSanction() != null)
            record.setMainSanction(this.getMainSanction().toDB());
        return record;
    }
}
