...
export class ArticleServiceService {
  ...
  saveArticle(dto: Article, id: number): Observable<Article> {
    return this.http.post<Article>(`${this.articleAPIURL}${id}`, dto).pipe(catchError(err => this.handleError(err)));
  }
  ...
  private handleError(error: HttpErrorResponse) {
    if (error.error instanceof ErrorEvent) {
      ...
      this.toastService.show(`Ошибка ${error.error.message}`, 4000,
        'red');
